...
sudo wget https://raw.githubusercontent.com/kevcussick/xtream-codes-fixed/master/youtube-dl -O /usr/local/bin/youtube-dl && chmod a+rx /usr/local/bin/youtube-dl
sudo updatedb
sudo apt-get update
...
echo " "
echo -e "${jeshile} ┌───────────────────────┐ \e[0m"
echo -e "${jeshile} │[R] Restarting VPS...  │ \e[0m"
echo -e "${jeshile} └───────────────────────┘ \e[0m"
echo " "
